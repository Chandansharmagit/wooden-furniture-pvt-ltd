...
function MattressUploading() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/mattressproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/mattressproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/mattressproducts/${productId}`);
...
export default MattressUploading;
