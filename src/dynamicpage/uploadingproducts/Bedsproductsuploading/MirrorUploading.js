...
function MirrorUploading() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/mirrorproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/mirrorproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/mirrorproducts/${productId}`);
...
export default MirrorUploading;
