...
function BedsProducts() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/bedsproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/bedsproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/bedsproducts/${productId}`);
