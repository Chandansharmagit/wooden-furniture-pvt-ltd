...
function PhotoFramesUploading() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/photoproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/photoproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/photoproducts/${productId}`);
...
export default PhotoFramesUploading;
