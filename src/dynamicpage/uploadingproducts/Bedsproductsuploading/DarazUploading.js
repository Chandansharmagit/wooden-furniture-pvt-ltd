...
function DarazUploading() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/darazproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/darazproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/darazproducts/${productId}`);
