...
function BedsideTable() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${apiBaseUrl}/BedSideproducts`);
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/BedSideproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/BedSideproducts/${productId}`);
        ...
        <div className="uploaded-products">
          {products.map((product) => (
            <div key={product._id} className="product-item">
              <img
                src={`${apiBaseUrl}${product.image1}`} // Use apiBaseUrl for images
                alt={product.name}
