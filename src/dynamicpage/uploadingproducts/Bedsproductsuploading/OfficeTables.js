...
function OfficeTables() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/Officeproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/Officeproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/Officeproducts/${productId}`);
