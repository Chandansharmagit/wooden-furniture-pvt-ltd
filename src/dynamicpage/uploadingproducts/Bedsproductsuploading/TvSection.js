...
function TvSectionUploading() {
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/tvproducts`).then((response) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${apiBaseUrl}/tvproducts/${productDetails._id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${apiBaseUrl}/tvproducts/${productId}`);
...
export default TvSectionUploading;
