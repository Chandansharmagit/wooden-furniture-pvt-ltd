...
function SecondTemplatesGrid() {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get("http://localhost:8080/gettingSeconds_templates_uploading");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await axios.put(`http://localhost:8080/updating_SecondsTemplate/${editImageId}`, formData, {
        ...
        // Upload new image
        await axios.post("http://localhost:8080/posting_SecondsTemplate", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
      setImage(null);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/deleting_SecondsTemplate/${id}`);
          ...
          {Array.isArray(images) && images.length > 0 ? (
            images.map((image) => (
              <div key={image.id} className="product-item">
                <img
                  src={`data:image/png;base64,${image.image1}`}
                  alt={image.link}
...
export default SecondTemplatesGrid;
